...
class FacebookAPI(GraphAPI):
    ...
    def _convert_facebook_data(cls, facebook_profile_data):
        ...
        user_data['major'] = FacebookAPI._retrieve_major(user_data)
    ...
    @classmethod
    def _retrieve_major(cls, facebook_data):
	    educations = facebook_data.get('education')
	    concentrations = ''
	    for idx in range(0, len(educations)):
		    if educations[idx]['type'] == 'College':
			   for conc in educations[idx]['concentration']:
