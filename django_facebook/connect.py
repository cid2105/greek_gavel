...
def _setProfilePicture(profile):
    fbname = profile.facebook_profile_url.replace('http://www.facebook.com/', '')
    filename= fbname + '.jpg'	
...
def _update_user(user, facebook):
    ...
    facebook_data = facebook.facebook_registration_data()
    facebook_fields = ['facebook_name', 'facebook_profile_url', 'date_of_birth', 'about_me', 'website_url', 'first_name', 'last_name', 'major']
    ...
    if hasattr(profile, 'raw_data'):
        ...
        profile_dirty = True
    if hasattr(profile, 'major'):
        profile.major = facebook_data['major']
        profile_dirty = True    
